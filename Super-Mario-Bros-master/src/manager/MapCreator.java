...
//tạo ra các anhr về các đối tượng trong trò chơi.tạo ra map , thiết lập các đối tương trong map.
class MapCreator {
    ...
    //hàm khởi tạo các giá trị hình ảnh trong map.
    ...
    Map createMap(String mapPath, double timeLimit) {
        ...
        //kích thước mỗi ô trên bản đồ là 48x48 pixel.
        int pixelMultiplier = 48;
        //các Biến này lưu giá trị mã màu của đối tượng Color vừa tạo.
        //Phương thức khởi tạo Color(int red, int green, int blue) tạo ra một đối tượng màu với 3 thành phần
        //các tham số trong hàm là cường độ của màu.
        //getRGB() :Lấy mã màu RGB của đối tượng Color dưới dạng một giá trị số nguyên (int).
        ...
        //vòng lặp for độ rộng của map đã cài đặt.
        //mapImage.getWidth() trả về chiều rộng của ảnh, tính bằng số pixel.
        //Vòng lặp này được sử dụng để duyệt qua từng pixel trong hình ảnh mapImage và
        // thực hiện các thao tác phân tích hoặc xử lý dựa trên màu sắc của từng pixel.
        for (int x = 0; x < mapImage.getWidth(); x++) {
            for (int y = 0; y < mapImage.getHeight(); y++) {
                //vị trí thực của đối tượng trên màn hình bằng vị trí của pixel nhân với kích thước của ô trên màn hình.
...
//        System.out.println(createdMap.getEndPoint().getY());
    ...
    private Prize generateRandomPrize(double x, double y){
        ...
        if (generated == null) {
            System.out.println("Error generating prize");
        }
